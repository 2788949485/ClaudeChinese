...
try {
    ...
    if ([System.IO.File]::ReadAllText($cliPath) -match 'title:"Tips for getting started"|title:"What''s new"|text:"Run /init') {
        throw "Claude CLI 欢迎页补丁未生效"
    }
    if (-not [System.IO.File]::ReadAllText($cliPath).Contains("TABLE: Tips for getting started | What's new | Run /init")) {
        throw "Claude CLI 补丁误改了二进制字符串表"
    }
